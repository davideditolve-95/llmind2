...
\begin{tikzpicture}[node distance=1.0cm and 1.1cm]
  ...
  \node[data, above right=of api] (postgres) {PostgreSQL\\casi, run, metriche};
  \node[data, below right=of api] (vector) {Vector store locale\\chunk ICD/CDDR/DSM};
  \node[box, right=of postgres] (etl) {Pipeline dati\\ICD API + PDF};
  \node[agent, right=of vector] (llm) {LLM locali\\Ollama / legacy RAG};
  \node[evalbox, below=of api] (benchmark) {Benchmark engine\\prompt, run, export};
  \node[evalbox, below=of vector] (metrics) {Metriche\\similarita, accuracy, P/R/F1};
  \node[data, right=of llm] (optional) {Estensioni opzionali\\GCP agenti gestiti};
...
\end{tikzpicture}}
\caption{Architettura implementativa del prototipo locale. Il nucleo riproducibile e composto da frontend, API, database relazionale, datastore vettoriale, pipeline di estrazione, LLM locali e motore di benchmark. Le integrazioni GCP sono estensioni opzionali di deployment e non dipendenze del protocollo scientifico.}
\label{fig:system-architecture-it}
\end{figure}
...
\toprule
\textbf{Metrica} & \textbf{Definizione operativa} & \textbf{Implementata}\\
\midrule
...
\bottomrule
\end{tabular}
\caption{Metriche automatiche del benchmark. Le misure lessicali sono euristiche riproducibili e devono essere interpretate insieme alla valutazione esperta, non come prova di validita clinica.}
\label{tab:automatic-metrics-it}
\end{table}
