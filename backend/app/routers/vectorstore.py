"""
Router per la gestione dei Vector Store (Chroma RAG offline).
...
from ..models.datastore import Datastore as VectorStoreModel
...
async def list_presets():
    """
    Elenca i preset di conoscenza disponibili per la creazione di nuovi vector store.
    """
    return [
...
async def create_vectorstore(
    ...
    """
    Avvia la creazione asincrona di un nuovo vector store locale basato su un preset di conoscenza.
    """
    ...
    if preset_id == "icd11_standard" and icd_scope:
        ...
        if nodes:
            generated_source = [
                "# ICD-11 Chapter 6 scoped vectorstore source\n",
                f"Scope: {icd_scope}",
            ...
            description = description or f"ICD-11 Chapter 6 scoped vectorstore ({icd_scope}) with {len(nodes)} nodes."
        else:
    ...
    new_datastore = VectorStoreModel(
        id=datastore_id,
        name=name,
...
async def list_vectorstores(db: Session = Depends(get_db)):
    """
    Elenca tutti i vector store creati.
    """
    datastores = db.query(VectorStoreModel).order_by(VectorStoreModel.created_at.desc()).all()
    return datastores

@router.delete("/{datastore_id}")
async def delete_vectorstore(datastore_id: uuid.UUID, db: Session = Depends(get_db)):
    """
    Rimuove un vector store e i suoi file associati.
    """
    datastore = db.query(VectorStoreModel).filter(VectorStoreModel.id == datastore_id).first()
    if not datastore:
...
async def ask_vectorstore(
    datastore_id: uuid.UUID, 
    request: AskRequest, 
    db: Session = Depends(get_db)
):
    """
    Interroga un vector store specifico usando la logica RAG.
    """
    datastore = db.query(VectorStoreModel).filter(VectorStoreModel.id == datastore_id).first()
    if not datastore:
        raise HTTPException(status_code=404, detail="Vector store not found")
    if datastore.status != "ready":
        raise HTTPException(status_code=400, detail=f"Vector store is not ready (status: {datastore.status})")
    ...
    try:
        ...
        logger.error(f"Errore query RAG su vectorstore {datastore_id}: {e}")
